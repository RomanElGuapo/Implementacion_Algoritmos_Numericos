...
if (found_x0):
  ...
  gx = sym.diff(user_function)
  decimals = int(input('Ingresa la cantidad de decimales de precicion que requieres para el metodo\t')) + 1
  convergence, i= 0,0
  last_x =  x0 - (user_function.evalf(decimals, subs = {x:x0}) /gx.evalf(decimals, subs = {x:x0}))

  while(convergence < 3):

    print(f'x{i} = {last_x}')

    next_x = last_x - (user_function.evalf(decimals, subs = {x:last_x}) /gx.evalf(decimals, subs = {x:last_x}))
